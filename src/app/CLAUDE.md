...
## API 라우트 (10) / API Routes

- `api/ai/route.ts` — AI 라우팅, 10단계 우선순위 (10-route priority, multi-route, SSE streaming)
- `api/steampipe/route.ts` — Steampipe 쿼리 + Cost 가용성 + 인벤토리 (POST/GET/PUT)
- `api/auth/route.ts` — 로그아웃 (HttpOnly 쿠키 삭제) (Sign Out: clear HttpOnly cookie)
- `api/msk/route.ts` — MSK 브로커 노드 + CloudWatch 메트릭 (kafka list-nodes, CW metrics)
- `api/rds/route.ts` — RDS 인스턴스 CloudWatch 메트릭 (CPU, Memory, IOPS, Network, Storage)
- `api/elasticache/route.ts` — ElastiCache 노드 CloudWatch 메트릭 (CPU, EngineCPU, Memory, Network, Connections)
- `api/opensearch/route.ts` — OpenSearch 도메인 CloudWatch 메트릭 (CPU, JVM, ClusterStatus, Search/Index rates)
- `api/agentcore/route.ts` — AgentCore Runtime/Gateway 상태 (config 기반, AWS CLI)
- `api/code/route.ts` — 코드 인터프리터 엔드포인트 (Code interpreter)
- `api/benchmark/route.ts` — CIS 컴플라이언스 벤치마크 (CIS benchmark)
